...
class front:
    ...
    def open_dados(self):
        os.popen(f'{config.dir_install}{config.softer_name}_{config.softer_version}/Dados.xlsx')

    def exec_top_btn_2(self):
        Thread(target=self.open_dados).start()
    ...
    def top(self):
        ...
        self.top_btn_2      = customtkinter.CTkButton(master=self.frame_top2,text='Dados',command=self.exec_top_btn_2)
        ...
        self.top_btn_2      .pack(padx=5,pady=5, side=tkinter.RIGHT)

        self.top_entry_1.insert(0, "5")
        self.top_entry_2.insert(0, "90")
