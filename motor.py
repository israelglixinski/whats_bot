...
class ignition:
    ...
    def __init__(self,feedback, janela, senha, action):
        ...
        if action == "start_bot": self.start_bot           ()
        pass
    ...
    def reinicializar(self):
        ...
        for pid in self.lista_pids_filhos:
            ...
            cont_pid+=1
        self.lista_pids_filhos   = []
        userstring = f'{self.usuario}*%@%*{self.senha}'
        crypto_userstring = Ferramentas.encrypta_fernet(userstring)
        os.popen(f'c:/Sistemas/Robos/{config.softer_name}_{config.softer_version}/{config.script_robo}.exe {crypto_userstring}')
    ...
    def gerencia(self):
        ...
        loop_time = timedelta(seconds=90)
